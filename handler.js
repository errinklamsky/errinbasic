   ...
   try {
      ...
      if (body && prefix && commands.includes(command) || body && !prefix && commands.includes(command) && setting.noprefix || body && !prefix && commands.includes(command) && env.evaluate_chars.includes(command)) {
         ...
         for (let name in is_events) {
            ...
            if (event.limit && !event.game && users.limit < 1 && body && Func.generateLink(body) && Func.generateLink(body).some(v => Func.socmed(v))) return client.reply(m.chat, `⚠️ Maaf kawan, kuota bertanyamu telah habis. Silahkan coba lagi besok ya.. kuota 10x/hari 😊.`, m).then(() => {
               users.premium = false
               users.expired = 0
            })
